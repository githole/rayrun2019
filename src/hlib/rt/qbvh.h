...
public:
    ...
    SIMDTrianglePack* simdTris_data;
    uint32_t simdTris_data_size = 0;

//	std::vector<SIMDBVHNode*> simdNodes;
    SIMDBVHNode* simdNodes_data;
    uint32_t simdNodes_data_size = 0;
//	std::vector<SIMDBVHNode, AlignmentAllocator<SIMDBVHNode, 16> > simdNodes_;
	...
	BVHBuildNode* recursiveBuild(int depth, std::vector<BVHPrimitiveInfo> &buildData, int start, int end, int *totalNodes, std::vector<RefTriangle*> &orderedPrims) {
		(*totalNodes) ++;
		...
		if (nPrimitives <= 4) {
            ...
            auto* simdt = &simdTris_data[simdTris_data_size++];
			...
			node->InitLeaf(firstPrimOffset, nPrimitives, bbox, simdTris_data_size - 1);
//			node->InitLeaf(firstPrimOffset, nPrimitives, bbox, NULL);
            ...
            BVHBuildNode* p0 = nullptr;
            BVHBuildNode* p1 = nullptr;
            auto build0 = [&]() {
                p0 = recursiveBuild(depth + 1, buildData, start, mid, totalNodes, orderedPrims);
            };
            auto build1 = [&]() {
                p1 = recursiveBuild(depth + 1, buildData, mid, end, totalNodes, orderedPrims);
            };
#if 0
            // 並列化、今は無理
            if (depth >= 5)
            ...
            {
                std::thread th0(build0);
                std::thread th1(build1);
            ...
            node->InitInterior(dim, p0, p1);


            /*
			node->InitInterior(dim,
				recursiveBuild(depth + 1, buildData, start, mid, totalNodes, orderedPrims),
				recursiveBuild(depth + 1, buildData, mid, end, totalNodes, orderedPrims));
            }
            */
		}
	...
	void collapse2QBVH(BVHBuildNode* node) {
        ...
        SIMDBVHNode* n = &simdNodes_data[simdNodes_data_size++];
		...
		for (int i = 0; i < 4; ++i) {
			if (c[i] == NULL) {
				...
				if (c[i]->nPrimitives == 0) {
					...
					n->children[i].node.index= simdNodes_data_size;
					collapse2QBVH(c[i]);
				} else {
	...
	void build(std::vector<RefTriangle>& atris) {
        // 大きめに確保
        simdTris_data = (SIMDTrianglePack*)_aligned_malloc(sizeof(SIMDTrianglePack) * atris.size(), 16);
        simdNodes_data =(SIMDBVHNode*)_aligned_malloc(sizeof(SIMDBVHNode) * atris.size(), 16);
...
//		tris = atris;
		...
		rootNode = recursiveBuild(0, buildData, 0, tris.size(), &totalNodes, orderedTris);

		// collapse
		collapse2QBVH(rootNode);

		SIMDBVHNode *root = &simdNodes_data[0];
	...
	bool intersect(const Ray &ray, Hitpoint* hitpoint, bool anyhit = false, float near_t = 0, float far_t = 0) const {
		...
		const SIMDBVHNode* nodes = &simdNodes_data[0];
		...
		while (todoNode >= 0) {
			...
			if(item.node.flag == 0){
				const SIMDBVHNode& node = (simdNodes_data[item.node.index]);
				...
				int nohitmask;
				SIMDTrianglePack *s = &simdTris_data[item.leaf.index];
